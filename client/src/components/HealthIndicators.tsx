  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-foreground">Health Indicators</h3>
          <p className="text-sm text-muted-foreground mt-1">
            Latest measurements and analysis
          </p>
        </div>
        <Badge variant="secondary" data-testid="badge-indicator-count">
          {indicators.length} Indicators
        </Badge>
      </div>

      <Accordion type="multiple" className="space-y-3" defaultValue={Object.keys(categorizedIndicators)}>
        {Object.entries(categorizedIndicators).map(([category, categoryIndicators]) => {
          ...
          return (
            <AccordionItem key={category} value={category} className="border-none">
              <Card>
                <AccordionTrigger className="px-6 py-4 hover:no-underline" data-testid={`accordion-${category.toLowerCase().replace(/\s+/g, '-')}`}>
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <h5 className="font-medium text-foreground" data-testid={`text-indicator-name-${indicator.indicator_id}`}>
                                  {indicator.indicator_name}
                                </h5>
                                {indicator.is_abnormal && indicator.abnormality_level && (
                                  <Badge
                                    variant="secondary"
                                    className={getSeverityColor(indicator.abnormality_level)}
                              ...
                              <div className="flex items-baseline gap-2">
                                <span className="text-2xl font-bold text-foreground" data-testid={`text-value-${indicator.indicator_id}`}>
                                  {indicator.value}
                                </span>
